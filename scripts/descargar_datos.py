...
#Script para descargar los datos que ya he obtenido con la maqqueta por la gente no puede generar datos, poder usar distintos csvs que ya tenemos del proyecto

MANIFEST_PATH = Path("datos/manifiestos/datasets.json")
...
def main():
    if not MANIFEST_PATH.exists():
        print(f"Error: No he podido encontrar el archivo de manifiesto en la ruta {MANIFEST_PATH}")
        return 1
        ...
        if ruta_destino.exists():
            if "tamano_bytes" in info and ruta_destino.stat().st_size != int(info["tamano_bytes"]):
                ...
                if hash_actual == info["sha256"]:
                    print(f"El archivo {ruta_relativa} ya existe y su hash es totalmente correcto.")
                    continue
                else:
                    print(f"Atención: El hash no coincide para {ruta_relativa}, así que vamos a volver a descargarlo.")

        print(f"Procediendo a descargar el dataset {ruta_relativa}...")
